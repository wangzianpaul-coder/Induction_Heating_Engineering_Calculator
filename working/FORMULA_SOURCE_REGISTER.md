...
| J-05 | 线圈-保温同心环隙 | definition + incomplete correlation | `ID-ANN-01`; ANNULUS-CASE PDF 1-2,4-6,10,13,15-16 只覆盖特定 K/aspect/Ra*；WB-FINAL images12-14 无原页 | 工作簿 `Ra=gβΔTgap³/(να)`, `Nu=0.59Ra^1/4` | `Ra=gβ|ΔT|Lc³/(να)`; `Nu=hLc/k`; 具体 `Nu(...)` 未批准 | V/R0 | R/VFY（定义 A；现关联式 X；需匹配环隙文献/试验） |
